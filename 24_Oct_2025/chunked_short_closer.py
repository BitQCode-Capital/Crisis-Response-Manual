...
def close_short_in_chunks(symbol="SOL/USDT", total_qty=8486.84, chunk_size=400):
    order_side = "buy"          # opposite of short
    position_side = "SHORT"     # required for hedge mode
    remaining = total_qty
    chunk = 1

    print(f"⚙️  Closing {total_qty} {symbol} short in {chunk_size}-SOL chunks...")
    ...
    while remaining > 0:
        ...
        try:
            order = exchange.create_market_order(
                ...
                params={
                    "positionSide": position_side   # ← keep only this
                }
            )
...
if name == "main":
    close_short_in_chunks("SOL/USDT", 8486.84, chunk_size=400)
